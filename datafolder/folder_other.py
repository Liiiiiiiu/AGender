...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 14 16:26:46 2021
...
from .reid_dataset import import_session
from .reid_dataset import attibute_other_data
...
class Train_Dataset(data.Dataset):
    ...
    def __init__(self, data_dir, dataset_name, transforms=None, train_val='train' ):
        ...
        print('~~train practice~~:',train)
        '''
        train, query, gallery = import_session.import_Session_nodistractors(data_dir, dataset_name)
        if dataset_name == 'attri_bot_cox':
            train_attr, query_attr,test_attr, self.label = attibute_other_data.import_SessionAttribute(data_dir,dataset_name)
        else:
...
class Test_Dataset(data.Dataset):
    def __init__(self, data_dir, dataset_name, transforms=None, query_gallery='query' ):
        ...
        if query_gallery == 'query':
            ...
            self.test_ids = gallery['ids']
            self.test_attr = query_attr + test_attr
        else:
