...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 11 18:49:49 2021
...
num_ids_dict = { 'market':751, 'duke':702 ,'attri_bot_cox':10000}
...
labels_tensor_age = labels_tensor[:,0:3]
preds_tensor_true = np.delete(preds_tensor_beard,beard_err,axis=0)
labels_tensor_true = np.delete(labels_tensor_age,age_err,axis=0)
...
def plot_confusion_matrix(cm,savename,classes,title='Confusion Matrix'):
    ...
    plt.show()
labels_age_delete = delete_ele(np_transform(labels_tensor_age),4)
preds_age_delete =  delete_ele(np_transform_age(preds_tensor_age,age_err),4)
for i in range(len(preds_age_delete)):
    if preds_age_delete[i] == 5:
        preds_age_delete[i] = 4
print('&&&&',preds_age_delete[0:1000])
classes_age = classes[0:4]
classes_age1 = classes_age.append('errors')
cm = confusion_matrix(labels_age_delete,preds_age_delete)
plot_confusion_matrix(nomalization(cm),'age_version_true_last.png',classes_age)

labels_downcolor_delete = delete_ele(np_transform(labels_tensor_downcolor),7)
preds_downcolor_delete =  delete_ele(np_transform_age(preds_tensor_downcolor,downcolor_err),7)
classes_dc = classes[4:11]
#classes_dc.pop(2)
classes_dc.append('errors')
cm_dc = confusion_matrix(labels_downcolor_delete,preds_downcolor_delete)
plot_confusion_matrix(nomalization(cm_dc),'downcolor_version_true_last.png',classes_dc)
labels_upcolor_delete = delete_ele(np_transform(labels_tensor_upcolor),7)
preds_upcolor_delete =  delete_ele(np_transform_age(preds_tensor_upcolor,upcolor_err),7)
classes_upcolor = classes[11:18]
classes_upcolor.append('errors')
cm_uc = confusion_matrix(labels_upcolor_delete,preds_upcolor_delete)
plot_confusion_matrix(nomalization(cm_uc),'upcolor_version_true_last.png',classes_upcolor)
labels_up_delete = labels_up_true.flatten()
preds_up_delete = preds_up_true.flatten()
classes_up = ['long','short']
cm_up = confusion_matrix(labels_up_delete,preds_up_delete)
plot_confusion_matrix(nomalization(cm_up),'up_version_last.png',classes_up)
labels_down_delete = labels_down_true.flatten()
preds_down_delete =  preds_down_true.flatten()
classes_down = classes[22]
cm_down = confusion_matrix(labels_down_delete,preds_down_delete)
plot_confusion_matrix(nomalization(cm_down),'down_version_last.png',classes_up)
labels_hair_delete = delete_ele(np_transform(labels_hair_true),4)
preds_hair_delete =  delete_ele(np_transform(preds_hair_true),4)
classes_hair = classes[20:24]
cm_hair = confusion_matrix(labels_hair_delete,preds_hair_delete)
plot_confusion_matrix(nomalization(cm_hair),'hair_version_last.png',classes_hair)
labels_glass_delete = labels_glass_true.flatten()
preds_glass_delete = preds_glass_true.flatten()
classes_glass = ['glasses','no glasses']
cm_glass = confusion_matrix(labels_glass_delete,preds_glass_delete)
plot_confusion_matrix(nomalization(cm_glass),'glass_version_last.png',classes_glass)
labels_gender_delete = labels_gender_true.flatten()
preds_gender_delete = preds_gender_true.flatten()
classes_gender = ['male','female']
cm_gender = confusion_matrix(labels_gender_delete,preds_gender_delete)
plot_confusion_matrix(nomalization(cm_gender),'gender_version_50.png',classes_gender)
