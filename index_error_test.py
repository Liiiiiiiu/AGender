...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 13 15:41:46 2021
...
preds_real = np.empty(shape=[0, num_label], dtype=np.byte) # # shape = (num_sample, num_label)
# Iterate over data.
with torch.no_grad():
    for count, (images, labels, ids, file_name) in enumerate(test_loader):
        ...
        pred_real = pred_label.cpu().numpy()
        labels = labels.cpu().numpy()
        preds = preds.cpu().numpy()
        # append
        preds_real = np.append(preds_real,pred_real,axis=0)
...
def correct(array,err_array,real):
    index = []
    for i in range(len(err_array)):
        index = np.argmax(real[err_array[i]])
        print('!!@@##',index)
        for j in range(len(array[err_array[i]])):
            if j == index:
                array[err_array[i]][j] = 1
            else:
                array[err_array[i]][j] = 0
    return array
...
preds_real_beard = preds_real[:,0:3]

preds_tensor_true = correct(preds_tensor_beard,beard_err,preds_real_beard)
print('##$$',preds_tensor_true.shape)
labels_tensor_true = labels_tensor_beard
print('@@!!',labels_tensor_true.shape)

preds_tensor_glass = preds_tensor[:,3:6]
labels_tensor_glass = labels_tensor[:,3:6]
preds_real_glass = preds_real[:,3:6]
preds_glass_true = correct(preds_tensor_glass,glass_err,preds_real_beard)
labels_glass_true = labels_tensor_glass

preds_tensor_up = preds_tensor[:,6:8]
labels_tensor_up = labels_tensor[:,6:8]
preds_real_up = preds_real[:,6:8]
preds_up_true = correct(preds_tensor_up,up_err,preds_real_up)
labels_up_true = labels_tensor_up

preds_tensor_down = preds_tensor[:,8:10]
labels_tensor_down = labels_tensor[:,8:10]
preds_real_down = preds_real[:,8:10]
preds_down_true = correct(preds_tensor_down,down_err,preds_real_down)
labels_down_true = labels_tensor_down

preds_tensor_age = preds_tensor[:,10:16]
labels_tensor_age = labels_tensor[:,10:16]
preds_real_age = preds_real[:,10:16]
preds_age_true = correct(preds_tensor_age,age_err,preds_real_age)
labels_age_true = labels_tensor_age

preds_tensor_gender = preds_tensor[:,16:18]
labels_tensor_gender = labels_tensor[:,16:18]
preds_real_gender = preds_real[:,16:18]
preds_gender_true = correct(preds_tensor_gender,gender_err,preds_real_gender)
labels_gender_true = labels_tensor_gender

preds_tensor_upcolor = preds_tensor[:,18:25]
labels_tensor_upcolor = labels_tensor[:,18:25]
preds_real_upcolor = preds_real[:,18:25]
preds_upcolor_true = correct(preds_tensor_upcolor,upcolor_err,preds_real_upcolor)
labels_upcolor_true = labels_tensor_upcolor

preds_tensor_downcolor = preds_tensor[:,25:32]
labels_tensor_downcolor = labels_tensor[:,25:32]
preds_real_downcolor = preds_real[:,25:32]
preds_downcolor_true = correct(preds_tensor_downcolor,downcolor_err,preds_real_downcolor)
labels_downcolor_true = labels_tensor_downcolor

preds_tensor_hair = preds_tensor[:,32:36]
labels_tensor_hair = labels_tensor[:,32:36]
preds_real_hair = preds_real[:,32:36]
preds_hair_true = correct(preds_tensor_hair,hair_err,preds_real_hair)
labels_hair_true = labels_tensor_hair
...
plot_confusion_matrix(nomalization(cm),'beard_version_real_180epoch.png',classes_beard)
...
plot_confusion_matrix(nomalization(cm_glass),'glasses_version_180epoch.png',classes_glass)
...
plot_confusion_matrix(nomalization(cm_up),'up_version_180epoch.png',classes_up)
...
plot_confusion_matrix(nomalization(cm_down),'down_version_180epoch.png',classes_down)
...
plot_confusion_matrix(nomalization(cm_age),'age_version_180epoch.png',classes_age)
...
plot_confusion_matrix(nomalization(cm_gender),'gender_version_180epoch.png',classes_gender)
...
plot_confusion_matrix(nomalization(cm_upcolor),'upcolor_version_180epoch.png',classes_upcolor)
...
plot_confusion_matrix(nomalization(cm_hair),'hair_version_180epoch.png',classes_hair)
...
plot_confusion_matrix(nomalization(cm_downcolor),'downcolor_version_180epoch.png',classes_downcolor)
